                    ...
                    if($latestNews != null){
			            ...
			            foreach ($latestNews as $latestNews1) {
                            ...
                            if($latestNews1->new_label){
                                $newStatus = anchor('aicte/updateLatestNewsLabel/'.$latestNews1->id.'/0','<i class="fa fa-bell"></i>','class="text-danger"');
                            }else{
                                $newStatus = anchor('aicte/updateLatestNewsLabel/'.$latestNews1->id.'/1','<i class="fa fa-bell"></i>','class="text-gray"');
                            }

                            if($latestNews1->status){
                                $status = anchor('aicte/updateLatestNewsStatus/'.$latestNews1->id.'/0','<i class="fa fa-check-circle"></i>','class="text-success"');
                            }else{
                                $status = anchor('aicte/updateLatestNewsStatus/'.$latestNews1->id.'/1','<i class="fa fa-check-circle"></i>','class="text-gray"');
                            }
                            ...
                            $actionItems = ' <div class="btn-group">
                                    ...
                                    <button type="button" class="btn btn-sm btn-light" data-toggle="tooltip" title="View">'.anchor('aicte/viewLatestNews/'.$latestNews1->id,'<i class="fa fa-fw fa-eye"></i>','class="text-dark"').'</button>
                            ...
                            $this->table->add_row($i++,
							        anchor('aicte/viewLatestNews/'.$latestNews1->id, $latestNews1->news_title),
